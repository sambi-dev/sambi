...
export function Stock({ props: { symbol, price, delta } }: { props: Stock }) {
  ...
  return (
    <div className="rounded-xl border bg-card p-4 text-primary shadow-md">
      <div className="float-right inline-block rounded-full border bg-background px-2 py-1 text-xs">
        {`${delta > 0 ? '+' : ''}${((delta / price) * 100).toFixed(2)}% ${
          delta > 0 ? '↑' : '↓'
        }`}
      </div>
      <div className="text-lg text-secondary-foreground">{symbol}</div>
      <div className="text-3xl font-bold">${price}</div>
      <div className="text mt-1 text-xs text-muted-foreground">
        As of close: 4:59 PM PST on Feb 27, 2055
      </div>
      ...
      >
        {priceAtTime.x > 0 ? (
          <div
            className="pointer-events-none absolute z-10 flex w-fit select-none gap-2 rounded-md bg-background p-2"
            style={{
        ...
        {startHighlight ? (
          <div
            className="pointer-events-none absolute h-32 w-5 select-none rounded-md border bg-muted-foreground/20"
            style={{
              left: startHighlight,
              width: endHighlight
        ...
        <div className="flex justify-end px-6 text-[0.6rem] text-muted-foreground/80">
          Remember, this data is all horseshit.
        </div>
      </div>
    </div>
  );
